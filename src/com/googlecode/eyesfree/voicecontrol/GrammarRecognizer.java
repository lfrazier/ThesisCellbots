...
public class GrammarRecognizer {
	 ...
	 * 
	 * @param context
	 *            The context that will own this object.
	 */
	 ...
	 * 
	 * @param resId
	 *            The resource ID of the G2G-formatted grammar.
	 * @return <code>true</code> on success
	 */
	 ...
	 * 
	 * @param grammar
	 *            A file representing the compiled G2G-formatted grammar.
	 * @return <code>true</code> on success
	 */
	...
	public boolean loadSlots(GrammarMap map) {
		...
		for (Entry<String, List<GrammarEntry>> pair : map.getEntries()) {
			...
			for (GrammarEntry entry : entries) {
				mSrecGrammar.addWordToSlot(slot, entry.word, entry.pron,
						entry.weight, entry.tag);
			}
		}
	 ...
	 * 
	 * @param resId
	 *            The resource ID of the raw grammar resource.
	...
	private TreeSet<GrammarResult> getRecognitionResults() {
		...
		for (int index = 0; index < mSrec.getResultCount()
				&& index < RESULT_LIMIT; index++) {
			int confidence = Integer.parseInt(mSrec.getResult(index,
			...
			GrammarResult result = new GrammarResult(meaning, literal,
					confidence);
	...
	public static interface GrammarListener {
		 ...
		 * 
		 * @param results
		 *            a set of results sorted by highest confidence first
		 */
		 ...
		 * 
		 * @param reason
		 *            a String explaining why recognition failed
		 */
	...
	public static class GrammarResult implements Comparable<GrammarResult> {
		 ...
		 * 
		 * @param meaning
		 *            the constructed meaning of the literal result
		 * @param literal
		 *            exactly what the speech recognition engine heard
		 * @param confidence
		 *            integer representing the recognition confidence
		 */
	...
	public static class GrammarMap {
		 ...
		 * 
		 * @param slot
		 *            the slot to assign the word to, ex. "Names" (not "@Names")
		 * @param word
		 *            the word to listen for, ex. "Alan Viverette"
		 * @param pron
		 *            (optional) word pronunciation in SREC format
		 * @param weight
		 *            (optional) word weight relative to other slot words
		 * @param tag
		 *            the meaning assigned to this word
		 */
		 ...
		 *            the slot to assign the word to, ex. "Names" (not "@Names")
		 * @param words
		 *            array of words to listen for, ex. "Alan Viverette"
		 * @param prons
		 *            (optional) array of word pronunciations in SREC format
		 * @param weights
		 *            (optional) array of word weights relative to other slot
		 *            word weights; use null for default
		 * @param tag
		 *            the meaning assigned to this word
		 */
		public void addWords(String slot, String[] words, String[] prons,
				int[] weights, String tag) {
