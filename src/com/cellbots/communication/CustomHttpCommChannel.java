...
public class CustomHttpCommChannel extends AbstractCommChannel {
	...
	@Override
	public void listenForMessages(final long waitTimeBetweenPolling,
					...
					try {
						...
						if (returnStream) {
							if (mMessageListener != null) {
								mMessageListener.onMessage(new CommMessage(
										null, cn.getInputStream(), null, null,
										null, mChannelName,
										CommunicationManager.CHANNEL_HTTP));
							}
						} else {
							...
							if (cmd != null && !cmd.equals(prevCmd)
								...
								if (cmd.indexOf(' ') >= 0) {
									try {
										Long.parseLong(cmd.substring(0,
												cmd.indexOf(' ')));
										cmd = cmd.substring(cmd.indexOf(' ') + 1);
									} catch (NumberFormatException e) {
									}
								}
								mMessageListener.onMessage(new CommMessage(cmd,
										null, null, null, null, mChannelName,
										CommunicationManager.CHANNEL_HTTP));
							}
						}
						if (waitTimeBetweenPolling < 0) { // Do not repeat this
															// loop
							break;
						}
						...
						Log.e(TAG, "Error reading command from URL: "
								+ commandUrl + " : " + e.getMessage());
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
				}
	...
	public void disconnect() {
		...
		if (mMessageListener != null)
			mMessageListener.onDisconnected(mChannelName,
					CommunicationManager.CHANNEL_HTTP);
	}
	...
	private class SendHttpTask extends UserTask<String, Void, Void> {
		...
		public Void doInBackground(String... params) {
			...
			if (params == null || params.length == 0 || mPutUrl == null
					|| mPutUrl.equals(""))
				return null;
			isReady = false;
			try {
				PutMethod put = new PutMethod(mPutUrl);
				put.setRequestBody(new ByteArrayInputStream(params[0]
						.getBytes()));
