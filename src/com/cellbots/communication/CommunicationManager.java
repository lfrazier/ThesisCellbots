...
public class CommunicationManager {
	...
	private HashMap<String, AbstractCommChannel> mChannelMap = new HashMap<String, AbstractCommChannel>();
	 ...
	 *            Name of the channel.
	 * @param receivingUrl
	 *            Receive message by polling this URL.
	 * @param sendingUrl
	 *            Send messages by PUTing to this URL.
	 * @param listener
	 *            The listener to received callback for this HTTP channel.
	 */
	public void addHttpChannel(String name, String receivingUrl,
			String sendingUrl, CommMessageListener listener) {
		mChannelMap.put(name, new CustomHttpCommChannel(receivingUrl,
				sendingUrl, listener, name));
	}
	 ...
	 *            Name of the channel.
	 * @param username
	 *            Username with which to sign-in.
	 * @param passwd
	 *            Account password.
	 * @param listener
	 *            The listener to received callback for this XMPP channel.
	 */
	public void addXmppCommChannel(String name, String username, String passwd,
		...
		mChannelMap.put(name, new XmppCommChannel(username, passwd,
				targetUsername, listener, name));
	}
	 ...
	 *            Name of the channel.
	 * @param url
	 *            The base url for receiving and sending messages.
	 * @param listener
	 *            The listener to received callback for this XMPP channel.
	 */
	public void addAppEngineCommChannel(String name, String url,
	 ...
	 * 
	 * @param channelName
	 *            Channel name on which to listen for messages.
	 * @param pollWaitTime
	 *            Duration in ms to wait for before polling for the next
	 *            message.
	 * @param returnStream
	 *            Whether to return an InputStream for the received message
	 *            through the CommMessageListener.onMessage() callback.
	 * @return true if the request channel exists and the request for listening
	 *         went through successfully.
	 */
	public boolean listenForMessages(String channelName, long pollWaitTime,
			boolean returnStream) {
		if (!mChannelMap.containsKey(channelName))
			return false;
		mChannelMap.get(channelName).listenForMessages(pollWaitTime,
				returnStream);
		return true;
	}
	 ...
	 * 
	 * @param channelName
	 *            The channel name to send the message via.
	 * @param message
	 *            The message to send.
	 * @param type
	 *            The mime type of the message content, ex: "text/text",
	 *            "image/jpg", "text/html", etc.
	 * @return true, if the specified channel exists and the request for sending
