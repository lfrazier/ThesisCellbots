...
public class AppEngineCommChannel extends AbstractCommChannel {
	...
	public AppEngineCommChannel(String name, String cmdUrl,
	...
	@Override
	public void listenForMessages(long waitTimeBetweenPolling,
			...
			public void run() {
				...
				while (!stopReading) {
					try {
						...
						if (cmdStart != -1) {
							String command = response.substring(cmdStart
									+ startCmdStr.length());
							command = command.substring(0,
									command.indexOf("\""));
							Log.e("command", command);
							mMessageListener.onMessage(new CommMessage(command,
									null, "text/text", null, null,
									mChannelName,
									CommunicationManager.CHANNEL_GAE));
						}
						// Thread.sleep(200);
	...
	private void resetConnection() {
		...
		try {
			mConnection = new HttpConnection(new URL(mHttpCmdUrl).getHost(),
					port);
			mConnection.open();
		} catch (MalformedURLException e) {
			e.printStackTrace();
