...
/**
 * Base class that includes methods useful for various robot profile creation
 * activities.
 * 
...
public class RobotBaseCreate extends Activity {
	...
	protected static final Uri BARCODE_SCANNER_MARKET_URI = Uri
	...
	protected String[] mCommChoices = new String[] { /* "AppEngine", */
	 ...
	 * getting the list of robots available, determining if this was an edit or
	 * create call, getting the name passed to us (if available), etc.
	 */
	@Override
	public void onCreate(Bundle savedInstanceState) {
		...
		if (mExtras != null) {
			mIsEdit = mExtras.getBoolean(RobotSelectionActivity.EXTRA_EDIT,
					false);
	...
	/**
	 * Start up the barcode scanner. If the scanner can't be found, provide a
	 * dialog alert allowing the user to install via the marketplace
	 * 
	 * @param me
	 *            activity that is initiating the scanner
	 * @param id
	 *            layout id for the button which initiates the scan request
	 */
	...
	public void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		if ((requestCode != INTENT_START_SCANNER)
		...
		if (scannedResult == null || scannedResult.length() == 0) {
			// TODO(ptucker): i18n
			Toast.makeText(getApplicationContext(), "No ID found.",
	 ...
	 * were scanned in. Should be overridden by children who want the scanned in
	 * data.
	 * 
	 * @param scannedEntry
	 *            scanned in RobotEntry. Only the data from the scan is
	 *            included. The callee should take what they need and fill in
	 *            defaults for missing values that are needed.
	 */
	 ...
	 *            The Spinner
	 * @param list
	 *            list of strings. Cannot be null, but can be an array of zero
	 *            length
	 */
	...
	public static String[] getPairedBluetooth() {
		...
		for (BluetoothDevice d : devices) {
			deviceNames[i++] = RobotEntry.generateNameMacPair(d.getName(),
					d.getAddress());
		}
		return deviceNames;
	}
