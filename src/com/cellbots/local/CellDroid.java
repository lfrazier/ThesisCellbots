...
public class CellDroid {
	...
	public static final String ACTION_ROBOT_CONTROLLER = "com.cellbots.controller.ROBOT_CONTROLLER_SERVICE";
	...
	private CommMessageListener mCommMessageListener = new CommMessageListener() {
		...
		public void onMessage(CommMessage msg) {
			...
			if (mCelldroidListener == null
					|| !mCelldroidListener.onMessage(message)) {
	...
	private void sendCommandToController(String cmd) {
		if (mControllerService != null) {
			try {
				...
				Log.e(TAG,
						"Error sending command to robot controller service: "
								+ e.getMessage());
			}
		}
	}
	...
	public CellDroid(Context ct, CellDroidListener listener) {
		...
		commandProcessor = new AndroidCommandProcessor(ct,
				new ResponseListener() {
					@Override
					public void onResponseRequested(String response) {
						mCommManager.sendMessage("HTTP_COMMAND", response,
								"text/text");
					}
				});
	}
	...
	private void startController(ServiceInfo serviceInfo,
			final String robotName, final String robotBtName,
			final String robotBtAddr) {
		if (mControllerService == null) {
			mServiceConnection = new ServiceConnection() {
				public void onServiceConnected(ComponentName name,
						IBinder service) {
					mControllerService = IRobotControllerService.Stub
							.asInterface(service);
					try {
						mControllerService.connect(robotName, robotBtName,
								robotBtAddr);
					} catch (RemoteException e) {
						e.printStackTrace();
					}
	...
	public void connect(String agentId, String username, String password,
			String baseUrl, boolean useLocalServer) {
		if (username != null && password != null && !username.equals("")) {
			mCommManager.addXmppCommChannel("XMPP", username, password, null,
					mCommMessageListener);
		}
		if (baseUrl != null && !baseUrl.equals("") && !useLocalServer) {
			mCommManager.addHttpChannel("HTTP_COMMAND",
					baseUrl + "command.txt", baseUrl + "response.php",
					mCommMessageListener);
		}
		if (agentId != null && !agentId.equals("")) {
			mCommManager.addAppEngineCommChannel("APP_ENGINE",
					"http://botczar.appspot.com/" + agentId + "/device",
					mCommMessageListener);
		}
