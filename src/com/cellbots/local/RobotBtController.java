...
public class RobotBtController {
	 ...
	 * 
	 * @param robotName
	 *            The name of the robot as its GMail account.
	 * @param robotBtName
	 *            The name of the robot's BT device.
	 * @param robotBtAddr
	 *            The MAC address of the robot's BT device.
	 */
	public RobotBtController(String robotName, String robotBtName,
			String robotBtAddr) {
	...
	public boolean isConnected() {
		try {
			return (mBtDevice != null && mBtSocket != null && mBtSocket
					.getInputStream() != null);
		} catch (IOException e) {
			e.printStackTrace();
			return false;
	 ...
	 * The phone should first be paired with the Bluetooth device. If we have
	 * the bluetooth mac address, just use that directly. Otherwise search for a
	 * device based on the bluetooth name.
	 */
	public boolean startConnection() {
		...
		try {
			...
			Log.e(TAG,
					"Unable to connect to robot's Bluetooth." + e.getMessage());
			mBtSocket = null;
		}
