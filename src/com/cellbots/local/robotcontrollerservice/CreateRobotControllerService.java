 ...
 * 
 */
public class CreateRobotControllerService extends
		AbstractRobotControllerService {
	...
	public IBinder onBind(Intent intent) {
		if (intent.getComponent().getClassName()
				.equals(CreateRobotControllerService.class.getName())) {
			return mBinder;
		}
		return null;
	}
	 ...
	 */
	private final AbstractRobotControllerService.Stub mBinder = new AbstractRobotControllerService.Stub() {
		@Override
		public void connect(String username, String robotBtName,
				String robotBtAddr) {
			super.connect(username, robotBtName, robotBtAddr);
			robotController = new RobotBtController(username, robotBtName,
					robotBtAddr);
	...
	private byte[] getIRobotCreateCommand(String cmd) {
		...
		if ((tokens[0].equals("f") || tokens[0].equals("b")
				...
				&& tokens.length == 3) {
			return getFixedMotionCommand(
					tokens[0],
					Math.max(-500,
							Math.min(500, Integer.parseInt(tokens[1]) * 5)),
					Integer.parseInt(tokens[2]));
		} else if ((tokens[0].equals("fd") || tokens[0].equals("bd")
				|| tokens[0].equals("ld") || tokens[0].equals("rd"))
				&& tokens.length == 3) {
			moveSpeed = turnSpeed = Math.max(-500,
					Math.min(500, Integer.parseInt(tokens[1]) * 5));
			tokens[0] = tokens[0].charAt(0) + "";
			if (waiterThread != null) {
				waiterThread.interrupt();
			}
			waiterThread = new Thread(new MoveWaiter(
					Integer.parseInt(tokens[2])));
			waiterThread.start();
		}
		...
		if (tokens[0].equals("w")) {
			...
			try {
				left = Math.max(-500,
						Math.min(500, Integer.parseInt(tokens[1]) * 5));
				right = Math.max(-500,
						Math.min(500, Integer.parseInt(tokens[2]) * 5));
