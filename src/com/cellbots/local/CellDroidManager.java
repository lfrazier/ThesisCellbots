...
public class CellDroidManager {
	...
	public static String[] getControllers(Context ct) {
		...
		List<ResolveInfo> resolveInfos = mPackageManager.queryIntentServices(
				new Intent(CellDroid.ACTION_ROBOT_CONTROLLER),
		...
		for (ResolveInfo resolveInfo : resolveInfos) {
			controllers[i++] = resolveInfo.serviceInfo.loadLabel(
	 ...
	 * @param password
	 */
	public void connect(final String agentId, final String username,
			final String password, final String cmdUrl,
			final boolean useLocalServer) {
		if (mCellDroid != null) {
			Log.e("DEBUG agentId", agentId);
			mCellDroid.connect(agentId, username, password, cmdUrl,
					useLocalServer);
		}
	}
	...
	public void setController(String label, String robotBtName,
			String robotBtAddr) {
		if (mCellDroid != null) {
