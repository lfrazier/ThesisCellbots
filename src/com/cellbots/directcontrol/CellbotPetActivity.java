 ...
 */
public class CellbotPetActivity extends Activity implements UiEventListener,
		Callback {
	public static final String TAG = "Cellbot Pet";

	public static final int VOICE_RECO_CODE = 42;

	public static final String PERSONA_IDLE = "http://personabots.appspot.com/personas/tuby/idle.png";

	public static final String PERSONA_READY = "http://personabots.appspot.com/personas/tuby/ready.png";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Display disp = ((WindowManager) getSystemService(Context.WINDOW_SERVICE))
				.getDefaultDisplay();
		mVoiceView = new VoiceView(this, this, false, disp.getWidth(),
				disp.getHeight());
	...
	private void trackface(byte[] imageData) {
		YuvImage yuvImage = new YuvImage(imageData, previewFormat,
				previewWidth, previewHeight, null);
		yuvImage.compressToJpeg(r, 50, out); // Tweak the quality here - 50 for
												// detection
		imageBytes = out.toByteArray();
		previewImage = BitmapFactory.decodeByteArray(imageBytes, 0,
				imageBytes.length);
		if (fd == null) {
			fd = new FaceDetector(previewImage.getWidth(),
					previewImage.getHeight(), 1);
		}
	...
	public void switchToAccelerometerControl() {
		...
		Display disp = ((WindowManager) getSystemService(Context.WINDOW_SERVICE))
				.getDefaultDisplay();
		mUiView = new AccelerometerView(this, this, false, disp.getWidth(),
				disp.getHeight());
	...
	public void switchToDpadControl() {
		...
		Display disp = ((WindowManager) getSystemService(Context.WINDOW_SERVICE))
				.getDefaultDisplay();
		mUiView = new DpadView(this, this, false, disp.getWidth(),
				disp.getHeight());
		mControlsLayout.addView(mUiView);
	}
	...
	public void switchToVoiceControl() {
		...
		Display disp = ((WindowManager) getSystemService(Context.WINDOW_SERVICE))
				.getDefaultDisplay();
		mUiView = new VoiceView(this, this, false, disp.getWidth(),
				disp.getHeight());
		mControlsLayout.addView(mUiView);
	}
	...
	public void switchToJoystickControl() {
		...
		Display disp = ((WindowManager) getSystemService(Context.WINDOW_SERVICE))
				.getDefaultDisplay();
		mUiView = new JoystickView(this, this, false, disp.getWidth(),
				disp.getHeight());
		mControlsLayout.addView(mUiView);
	...
	public void onWheelVelocitySetRequested(float direction, float speed) {
		if (direction >= 0 && direction <= 90) {
			CellDroidManager.sendDirectCommand("w " + (int) speed + " "
					+ (int) (speed - direction / 90 * speed));
		} else if (direction > 90) {
			CellDroidManager.sendDirectCommand("w -" + (int) speed + " -"
					+ (int) (speed - (180 - direction) / 90 * speed));
		} else if (direction < 0 && direction >= -90) {
			CellDroidManager.sendDirectCommand("w "
					+ (int) (speed - Math.abs(direction) / 90 * speed) + " "
					+ (int) speed);
		} else if (direction < -90) {
			CellDroidManager.sendDirectCommand("w -"
