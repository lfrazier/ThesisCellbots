...
public class RobotEntry {
	 ...
	 */
	public enum Usage {
		UNKNOWN("Unknown"), DIRECT_CONTROL(
				"Phone is talking directly to the robot via bluetooth"), BRAIN(
				"Phone is acting as a brain and/or relay between the robot and the internet"), WEB_CONTROL(
				"Phone is talking via the internet to a relay on a robot");
	...
	private static final Pattern NAME_MAC_SPLITTER = Pattern
			.compile("(.*) \\[([a-zA-z0-9:]+)\\]$");
	...
	/**
	 * Allow an external entity that has a name and a mac generate the same
	 * string as the nameMacPair() above.
	 * 
	 * @param name
	 *            bluetooth name
	 * @param mac
	 ...
	 * 
	 * @param nameMac
	 *            String generated from generateNameMacPair
		...
		if (m.matches()) {
			if (m.groupCount() == 2) {
				...
				if (BluetoothAdapter.checkBluetoothAddress(m.group(2))) {
					...
					Log.w(TAG, "Problem splitting - bad mac address: "
							+ nameMac);
					ret[1] = "";
				}
			} else {
				Log.e(TAG, "Problem splitting - count wrong: " + nameMac + " ("
						+ m.groupCount() + ")");
				ret[0] = nameMac;
				ret[1] = "";
			}
	...
	public static RobotEntry newDirectRobotEntry(String name, String type,
			String bt, String btAddr) {
		return new RobotEntry(name, type, false, "!", "!", "!", false, "!", bt,
				Usage.DIRECT_CONTROL, btAddr);
	}

	public static RobotEntry newLocalRobotEntry(String name, String gmail,
			String passwd, String url, boolean useLocal, String agent,
			String type, String bt, boolean wifi, String btAddr) {
		return new RobotEntry(name, type, wifi, gmail, passwd, url, useLocal,
				agent, bt, Usage.BRAIN, btAddr);
	}

	public static RobotEntry newRemoteRobotEntry(String name, String gmail,
			String url, String agent) {
		return new RobotEntry(name, "!", false, gmail, "!", url, false, agent,
				"!", Usage.WEB_CONTROL, "!");
	}
	...
	public RobotEntry(String nameStr, String cellbotTypeStr, boolean wifiBool,
			String gmailStr, String passwdStr, String urlStr, boolean useLocal,
			String agentIdStr, String btStr, Usage usage, String btAddr) {
	...
	private void updateUrl() {
		url = useLocalHttp ? "http://"
				+ HttpCommandServerServiceManager.getLocalIpAddress()
				+ ":8080/" : url;
	}

	/**
	 * Serialize a robot entry based on the usage. Leave out fields that aren't
	 * necessary for a scanner (i.e. leave out passwords)
	 * 
	 * @return
	 */
	public String serializeForUsage() {
		...
		case DIRECT_CONTROL:
			s = name + "|" + cellbotType + "|" + "!" + "|" + "!" + "|" + "!"
					+ "|" + "!" + "|" + "!" + "|" + bluetoothName + "|"
					+ intendedUsage.ordinal() + "|" + "!" + "|" + bluetoothAddr;
			break;
		case BRAIN:
		...
		case WEB_CONTROL:
			s = name + "|" + "!" + "|" + "!" + "|" + gmail + "|" + "!" + "|"
					+ url + "|" + agentId + "|" + "!" + "|"
					+ intendedUsage.ordinal() + "|" + "!" + "|" + "!";
			break;
		default:
			Log.e(TAG, "Problem serializing: " + name + " bad usage "
					+ intendedUsage);
			s = "";
		}
		return s;
	 ...
	 * password. Someone who deserializes this should get an exact copy of the
	 * robot entry.
	 * 
	 * @return
	 */
	public String serialize() {
		updateUrl();
		return name + "|" + cellbotType + "|" + enableWiFi + "|" + gmail + "|"
				+ passwd + "|" + url + "|" + agentId + "|" + bluetoothName
				+ "|" + intendedUsage.ordinal() + "|" + useLocalHttp + "|"
				+ bluetoothAddr;
	}
