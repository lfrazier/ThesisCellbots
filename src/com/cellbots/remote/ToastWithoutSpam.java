...
public class ToastWithoutSpam {
	...
	public ToastWithoutSpam(Activity activity, Context context, int depth,
			int minTimeMs) {
		mActivity = activity;
		mContext = context;
		mDepth = depth;
		...
		try {
			...
			if (null != when) {
				if ((now - when) < mDelayTimeMs) {
					...
					Log.d(TOAST_TAG, "skipped: " + text + " now: " + now
							+ " when: " + when);
					return false;
				}
				...
				if (mPreviousToast.size() > mDepth) {
					...
					Log.d(TOAST_TAG, "removed: " + s[pick] + "size: "
							+ mPreviousToast.size());
				}
			}
