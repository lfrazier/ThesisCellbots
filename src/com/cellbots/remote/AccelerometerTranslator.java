...
public class AccelerometerTranslator {
	...
	public AccelerometerTranslator(Context ct, UiEventListener uiListener,
			long maxRateMs) {
		mContext = ct;
	...
	public void movement(SensorData currentValues) {
		...
		if (deltaRobotUpdateMs >= 100) {
			...
			if ((Math.abs(mSpeedX) < IDLE_SPEED_THRESHOLD_MMPS)
				...
				mcount = mcount + 1;
				if (mcount > 5) {
					Log.i(CellbotRCActivity.TAG, "Press time (ms): "
							+ relativeTimeMs);
					Log.i(CellbotRCActivity.TAG,
							" currentPitch: " + Math.toDegrees(currentTanYZ));
					Log.i(CellbotRCActivity.TAG,
							" relativePitch: " + Math.toDegrees(relPitch));
					Log.i(CellbotRCActivity.TAG,
							" currenRoll: " + Math.toDegrees(currentTanXZ));
					Log.i(CellbotRCActivity.TAG,
							" relativeRoll: " + Math.toDegrees(relRoll));
