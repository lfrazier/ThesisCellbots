...
public class AccelerometerView extends UiView {
	...
	public AccelerometerView(final Context ct,
			final UiEventListener uiListener, boolean drawer, int width,
			int height) {
		super(ct, uiListener);
		isDrawer = drawer;
		mContext = ct;
		...
		mSensorManager = (SensorManager) ct
				.getSystemService(Context.SENSOR_SERVICE);
		mSensorAccelerometer = mSensorManager
				.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);

		Log.v(CellbotRCActivity.TAG, "name: " + mSensorAccelerometer.getName());
		Log.v(CellbotRCActivity.TAG,
				" res: " + mSensorAccelerometer.getResolution());
		Log.v(CellbotRCActivity.TAG,
				" max: " + mSensorAccelerometer.getMaximumRange());
		Log.v(CellbotRCActivity.TAG, " type: " + mSensorAccelerometer.getType());
		Log.v(CellbotRCActivity.TAG,
				" vendor: " + mSensorAccelerometer.getVendor());
		Log.v(CellbotRCActivity.TAG,
				" version: " + mSensorAccelerometer.getVersion());

		mTranslator = new AccelerometerTranslator(ct, uiListener,
		...
		nextControl.setImageResource(R.drawable.brain_small);
		nextControl.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				uiListener
						.onSwitchInterfaceRequested(UiEventListener.INTERFACE_WAVE);
			}
		});
		...
		previousControl.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				uiListener
						.onSwitchInterfaceRequested(UiEventListener.INTERFACE_JOYSTICK);
			}
		});
	}
}
