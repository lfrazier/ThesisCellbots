...
public class HttpCommandServerServiceManager {
	...
	private ServiceConnection serviceConn = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			httpServerService = IHttpCommandServerService.Stub
					.asInterface(service);
			try {
	...
	private ServiceConnection registerCallbackConn = new ServiceConnection() {
		...
		@Override
		public void onServiceConnected(ComponentName name, IBinder service) {
			registerCallbackService = IRegisterCallbackService.Stub
					.asInterface(service);
			try {
	...
	public static void copyLocalServerFiles(Context ct) {
		String[] fileNames = ct.getResources().getStringArray(
				R.array.copy_files);
	...
	public HttpCommandServerServiceManager(Context ct,
			HttpRequestListener listener) {
		context = ct;
		requestListener = listener;
		HttpCommandServerServiceManager.copyLocalServerFiles(ct);
		context.bindService(
				new Intent(IHttpCommandServerService.class.getName()),
				serviceConn, Context.BIND_AUTO_CREATE);
		context.bindService(
				new Intent(IRegisterCallbackService.class.getName()),
	...
	public void setResponseByName(String name, byte[] data, String contentType) {
		if (httpServerService != null) {
			try {
				httpServerService
						.setResponseDataByName(name, data, contentType);
			} catch (RemoteException e) {
	...
	public void setResponseByName(String name, String resource,
			String contentType) {
		if (httpServerService != null) {
			try {
				httpServerService.setResponsePathByName(name, resource,
						contentType);
			} catch (RemoteException e) {
				Log.e(TAG, "Error calling remote method to set response.");
			}
	...
	private IHttpCommandServerServiceCallback mCallback = new IHttpCommandServerServiceCallback.Stub() {
		@Override
		public void OnRequest(String request, String[] keys, String[] values,
				byte[] data) {
